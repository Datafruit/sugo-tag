...
if __name__ == "__main__":
    ...
    data_home = TASK_DATA_HOME + "/algorithm_test/kaggle/mushroom-classification"
    df = pd.read_csv(data_home + "/mushrooms.csv")
    df.reset_index(inplace=True)
    id = "index"
    features_continuous = []
    features_discrete = filter(lambda x: x not in ("class", id), df.columns.tolist())
    df["__label__"] = df["class"].apply(lambda x: 1 if x == "p" else 0)
    label = "__label__"
